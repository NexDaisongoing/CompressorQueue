...
try:
    ...
    FFMPEG = config(
        "FFMPEG",
        default='ffmpeg -i "{}" -preset ultrafast -c:v libx264 -s 1080×540 -x264-params "bframes=8:psy-rd=1:ref=3:aq-mode=3:aq-strength=0.8:deblock=1,1" -metadata title="Encoded By Night Owls" -metadata:s:v title="Anime Sensei" -metadata:s:a title="The Night Owls" -metadata:s:s title="The Night Owls" -pix_fmt yuv420p -crf 28 -c:a libopus -b:a 32k -c:s copy -map 0 -ac 2 -ab 32k -vbr 2 -level 3.1 -threads 1 "{}"'
    )
